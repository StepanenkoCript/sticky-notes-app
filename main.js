...
function criarTarefa() {    
  ...
  const conteudo = `<div class="sticky-header"></div><textarea class="sticky-title" rows="1" cols="25" placeholder="Title..."></textarea><textarea rows="8" cols="25"></textarea>`; //tagged template literals
  notas.innerHTML = conteudo;  

     
  notas.appendChild(botaox)
  notas.ondragstart = function () { // browser has its own drag’n’drop support for images and some other elements. It runs automatically and conflicts with ours.To disable it:
    return false;
  };
  //============= END: create new note  ============//
...
const deletarTarefa = (e) => {
  const deletarSticky = e.target.parentElement;
